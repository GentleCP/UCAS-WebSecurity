...
### 使用docker部署
1. 下载本项目
2. `docker-compose up` ，程序会监听本机`40000`端口
...
- 攻击方法        
1.通过提供的数据库，取出其中出现频率较高的密码，并且按照频率由高到低存储为字典库；   
2.查看网页认证方式(用户名和密码登录)；   
3.根据网页认证方式和字典构造登录报文，并且启用多线程不断访问网页直到攻击成功。   

- 攻击结果   
攻击脚本同时开启5个线程，读取字典库，对网页进行爆破   
![](src/4.2-1.PNG)  
最后爆破成功，获取到test账户的密码为123321123    
### 4.3 限制ip登录与绕过方法  
- 限制方式  
...
### 4.6 限制用户名登录与绕过方法
- 限制方式
	- 设置时间段为1min，该时间段内，单个用户密码错误次数上限为5
	- 后台记录记录该段时间内第一次登录失败的时间戳，记录该段时间内单个账户登陆失败的次数
	- 从第一次登录失败的时间开始的1min为需要限制的时间段，如果单个账户登陆失败的次数到达上限5，则对该账号进行封禁。
